  ...
  async function handleCardCheckout() {
    ...
    try {
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({ items: $cart, email, name, total: $cartTotal })
      });
  ...
  async function handleCryptoCheckout() {
    ...
    try {
      const res = await fetch('/api/coinbase/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: $cart, email, currency: cryptoCurrency, total: $cartTotal })
      });
      const data = await res.json();
      if (data.hosted_url) {
                ...
                <div class="crypto-grid">
                  {#each cryptoCurrencies as coin}
                    <button
                      ...
                      on:click={() => cryptoCurrency = coin.id}
...
.field-note { font-size: 11px; color: var(--text3); font-family: var(--font-mono); }
...
.method-tab.active { border-color: var(--accent); color: var(--accent); background: rgba(200,241,53,0.05); }
.method-tab.crypto-tab.active { border-color: #1652f0; color: #6b9bff; background: rgba(22,82,240,0.08); }
...
.card-logos { display: flex; gap: 8px; flex-wrap: wrap; }
...
.crypto-intro { font-size: 14px; color: var(--text2); margin-bottom: 16px; line-height: 1.6; }
.crypto-intro strong { color: var(--text); }

.crypto-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 16px; }
...
.coin-btn.active { border-color: var(--coin-color); color: var(--coin-color); background: rgba(255,255,255,0.03); }
...
.os-title { font-size: 16px; font-weight: 700; margin-bottom: 20px; }
...
.os-item { display: flex; justify-content: space-between; align-items: flex-start; gap: 8px; }
.os-item-info { display: flex; flex-direction: column; gap: 2px; flex: 1; }
.os-item-name { font-size: 13px; color: var(--text); line-height: 1.3; }
.os-item-qty { font-family: var(--font-mono); font-size: 11px; color: var(--text3); }
.os-item-price { font-family: var(--font-mono); font-size: 13px; font-weight: 700; color: var(--text); white-space: nowrap; }
...
.os-row { display: flex; justify-content: space-between; font-family: var(--font-mono); font-size: 12px; color: var(--text2); }
.os-row .fee { color: var(--text3); }

.os-total { display: flex; justify-content: space-between; font-family: var(--font-mono); font-size: 18px; font-weight: 700; margin-bottom: 16px; }

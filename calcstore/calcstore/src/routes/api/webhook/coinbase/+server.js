 ...
 * Listens for Coinbase Commerce payment events.
 * Add this URL in your Coinbase Commerce Dashboard → Settings → Webhooks.
 */
export async function POST({ request }) {
  const sig = request.headers.get('x-cc-webhook-signature');
  const body = await request.text();

  // Verify Coinbase Commerce HMAC-SHA256 signature
  ...
  switch (type) {
    case 'charge:confirmed': {
      ...
      const charge = data;
      await fulfillCryptoOrder(charge);
      break;
    }
    ...
    case 'charge:delayed': {
      // Underpaid or slow confirmation — handle manually
      console.warn('[Coinbase] Charge delayed (underpaid?):', data.id);
      break;
    }

    case 'charge:resolved': {
      // Previously unresolved charge is now resolved
...
async function fulfillCryptoOrder(charge) {
  ...
  // Which crypto was actually used
  ...
  // await db.cryptoOrders.create({ chargeId: charge.id, email, items, ... });
}

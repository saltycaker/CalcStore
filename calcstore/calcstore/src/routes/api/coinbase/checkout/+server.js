 ...
 * Coinbase Commerce doesn't natively support split payouts,
 * so the full amount lands in your Coinbase Commerce account.
 * You then pay sellers their 95% cut manually or via your payout system.
 *
 * Supported crypto: BTC, ETH, USDC, SOL, LTC, DOGE, DAI, APE, SHIB, XRP, MATIC
 * (all currencies Coinbase Commerce accepts — customer selects at checkout).
 */
export async function POST({ request }) {
  try {
    ...
    // Build Coinbase Commerce charge via REST API
    const payload = {
      name: 'CalcMod Order',
      description,
      local_price: {
    ...
    return json({
      ...
      // Expose the crypto addresses so a custom UI could show them
